...
export default function ProjectDetail() {
  ...
  return (
    <>
      {/* Hero — header tối */}
      <section className="relative overflow-hidden border-b border-line pt-28 pb-10 sm:pt-36">
        <div className="absolute inset-0 -z-10 bg-grid-faint" style={{ backgroundSize: "40px 40px", opacity: 0.4 }} />
        <div className="container-x">
          <div className="flex items-center gap-2 font-mono text-xs text-ink-muted">
            <Link to="/" className="hover:text-neon">~</Link>
            <span>/</span>
            <Link to="/du-an" className="hover:text-neon">du-an</Link>
            <span>/</span>
            <span className="text-brand-300">{p.slug}</span>
          </div>
          ...
          >
            <span className="grid h-20 w-20 flex-none place-items-center rounded-xl border border-brand-500/30 bg-brand-500/10 font-mono text-3xl font-bold text-neon">
              0{p.id}
            </span>
            <div>
              <span className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider text-brand-400">
          ...
          <div className="mx-auto max-w-3xl">
                ...
                {p.submission ? (
                  <div className="space-y-5">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="inline-flex items-center gap-1.5 font-mono text-xs font-medium uppercase tracking-[0.12em] text-neon">
                  ...
                  <p className="border-l-2 border-brand-500/40 bg-white/[0.03] py-3 pl-4 text-sm text-ink-muted">
            ...
            <Reveal>
              <Section label="Trích nguồn & công cụ AI">
                <div className="border-l-2 border-neon bg-white/[0.03] p-5">
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="card-surface mb-5 p-6 sm:p-7">
      <h2 className="mb-4 font-mono text-xs font-medium uppercase tracking-[0.2em] text-neon">
        {label}
      </h2>
      {children}
    </div>
